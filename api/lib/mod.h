...
#define _TInstanceHook(class_inh, pclass, iname, sym, ret, ...)       \
    struct _TInstanceHook_##iname class_inh {                         \
        ...
        static ret original(pclass* _this, Params&&... params) {      \
            return (((_TInstanceHook_##iname*)_this)->*_original)(    \
                std::forward<Params>(params)...);                     \
        }                                                             \
        ret _hook(__VA_ARGS__);                                       \
    };                                                                \
    static RegisterStaticHook _TRInstanceHook_##iname(                \
        sym, &_TInstanceHook_##iname::_hook,                          \
        (void**)&_TInstanceHook_##iname::_original);                  \
    ret (_TInstanceHook_##iname::*_TInstanceHook_##iname::_original)( \
        __VA_ARGS__);                                                 \
    ret _TInstanceHook_##iname::_hook(__VA_ARGS__)
#define _TInstanceDefHook(iname, sym, ret, type, ...) \
    _TInstanceHook( : public type, type, iname, sym, ret, __VA_ARGS__)
#define _TInstanceNoDefHook(iname, sym, ret, ...) \
    _TInstanceHook(, void, iname, sym, ret, __VA_ARGS__)
...
#define _TStaticDefHook(iname, sym, ret, type, ...) \
    _TStaticHook( : public type, iname, sym, ret, __VA_ARGS__)
#define _TStaticNoDefHook(iname, sym, ret, ...) \
    _TStaticHook(, iname, sym, ret, __VA_ARGS__)

#define THook2(iname, ret, sym, ...) \
    _TStaticNoDefHook(iname, sym, ret, __VA_ARGS__)
#define THook(ret, sym, ...) THook2(sym, ret, sym, __VA_ARGS__)
#define TClasslessInstanceHook2(iname, ret, sym, ...) \
    _TInstanceNoDefHook(iname, sym, ret, __VA_ARGS__)
#define TClasslessInstanceHook(ret, sym, ...) \
    TClasslessInstanceHook2(sym, ret, sym, args)
#define TInstanceHook2(iname, ret, sym, type, ...) \
    _TInstanceDefHook(iname, sym, ret, type, __VA_ARGS__)
#define TInstanceHook(ret, sym, type, ...) \
    TInstanceHook2(sym, ret, sym, type, __VA_ARGS__)
#define TStaticHook2(iname, ret, sym, type, ...) \
    _TStaticDefHook(iname, sym, ret, type, __VA_ARGS__)
#define TStaticHook(ret, sym, type, ...) \
    TStaticHook2(sym, ret, sym, type, __VA_ARGS__)
